...
class InformationElementIdentifierEncoderTest(EncoderTest):
    ...
    def test_conversion(self):
        ...
        ), InformationElementIdentifier.CONCATENATED_SM_16BIT_REF_NUM, '08')
...
class IEConcatenatedSMEncoderTest(EncoderTest):
    ...
    def test_conversion(self):
        self.do_conversion_test(
            IEConcatenatedSMEncoder(False), IEConcatenatedSM(0xFA, 0x03, 0x02), 'fa0302')
        self.do_conversion_test(IEConcatenatedSMEncoder(
            True), IEConcatenatedSM(0x9CFA, 0x03, 0x02), '9cfa0302')
...
class InformationElementEncoderTest(EncoderTest):
    ...
    def test_conversion(self):
        self.do_conversion_test(InformationElementEncoder(), InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_8BIT_REF_NUM, IEConcatenatedSM(0xFA, 0x03, 0x02)), '0003fa0302')
        self.do_conversion_test(InformationElementEncoder(), InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_16BIT_REF_NUM, IEConcatenatedSM(0x9CFA, 0x03, 0x02)), '08049cfa0302')
        self.do_conversion_test(InformationElementEncoder(), InformationElement(
            InformationElementIdentifier.HYPERLINK_FORMAT_ELEMENT, binascii.a2b_hex('9cfa0302')), '21049cfa0302')

    def test_decode_unknown_identifier(self):
        decoded = self.decode(
            InformationElementEncoder().decode, '02049cfa0302')
        self.assertEquals(None, decoded)
        decoded = self.decode(InformationElementEncoder().decode, '0200')
        self.assertEquals(None, decoded)

    def test_invalid_length(self):
        self.do_decode_udh_parse_error_test(
            InformationElementEncoder().decode, '0002fa0302')
        self.do_decode_udh_parse_error_test(
            InformationElementEncoder().decode, '0004fa0302')
...
class UserDataHeaderEncoderTest(EncoderTest):
    ...
    def test_conversion(self):
        udh = [InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_8BIT_REF_NUM, IEConcatenatedSM(0x24, 0x03, 0x01))]
        self.do_conversion_test(UserDataHeaderEncoder(), udh, '050003240301')

    def test_decode_repeated_non_repeatable_element(self):
        udh = self.decode(
            UserDataHeaderEncoder().decode, '0c0804abcd030208049cfa0302')
        udhExpected = [InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_16BIT_REF_NUM, IEConcatenatedSM(0x9CFA, 0x03, 0x02))]
        self.assertEquals(udhExpected, udh)

    def test_decode_with_unknown_elements(self):
        udh = self.decode(
            UserDataHeaderEncoder().decode, '0f0203ffffff0201ff00032403010200')
        udhExpected = [InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_8BIT_REF_NUM, IEConcatenatedSM(0x24, 0x03, 0x01))]
        self.assertEquals(udhExpected, udh)
    ...
    def test_encode_repeated_non_repeatable_element(self):
        ...
        ie2 = InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_16BIT_REF_NUM, IEConcatenatedSM(0xABCD, 0x04, 0x01))
        udh = [ie1, ie2]
        self.assertRaises(ValueError, UserDataHeaderEncoder().encode, udh)

    def test_decode_mutually_exclusive_elements(self):
        udh = self.decode(
            UserDataHeaderEncoder().decode, '0b000324030108049cfa0302')
        udhExpected = [InformationElement(
            InformationElementIdentifier.CONCATENATED_SM_16BIT_REF_NUM, IEConcatenatedSM(0x9CFA, 0x03, 0x02))]
        self.assertEquals(udhExpected, udh)
