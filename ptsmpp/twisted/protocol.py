...
from ptsmpp.pdu.operations import *
from ptsmpp.pdu.pdu_encoding import PDUEncoder
from ptsmpp.pdu.pdu_types import PDURequest, PDUResponse, PDUDataRequest, CommandStatus
from ptsmpp.pdu.error import *
from ptsmpp.pdu.constants import command_status_name_map
...
LOG_CATEGORY="ptsmpp.twisted.protocol"
    ...
    def PDURequestFailed(self, error, reqPDU):
        if error.check(SMPPProtocolError):
            ...
            try:
                ...
                self.log.debug("Application raised error '%s', forwarding to client. Inbound PDU was [%s], hex[%s]" % (validation_error, _safelylogOutPdu(reqPDU), _safelylogOutPdu(self.encoder.encode(reqPDU))))
