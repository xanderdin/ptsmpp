...
"""
import struct
from ptsmpp.pdu import gsm_constants, gsm_types
from ptsmpp.pdu.encoding import IEncoder
from ptsmpp.pdu.error import PDUParseError
...
class InformationElementIdentifierEncoder(IEncoder):
    ...
    def encode(self, value):
        ...
        if name not in self.nameMap:
            raise ValueError(
                "Unknown InformationElementIdentifier name %s" % name)
...
class InformationElementEncoder(IEncoder):
    ...
    def encode(self, iElement):
        ...
        if iElement.identifier in self.dataEncoders:
            dataBytes = self.dataEncoders[
                iElement.identifier].encode(iElement.data)
        else:
    ...
    def decode(self, file):
        ...
        if parsed != length + 2:
            raise UDHParseError(
                "Invalid length: expected %d, parsed %d" % (length + 2, parsed))
...
class UserDataHeaderEncoder(IEncoder):
        ...
        for iElement in udh:
            if not self.isIdentifierRepeatable(iElement.identifier):
                if iElement.identifier in nonRepeatable:
                    raise ValueError(
                        "Cannot repeat element %s" % str(iElement.identifier))
                for identifier in self.getIdentifierExclusionList(iElement.identifier):
                    if identifier in nonRepeatable:
                        raise ValueError("%s and %s are mutually exclusive elements" % (
                            str(iElement.identifier), str(identifier)))
    ...
    def getIdentifierExclusionList(self, identifier):
        nameList = gsm_constants.information_element_identifier_full_value_map[
            gsm_constants.information_element_identifier_name_map[str(identifier)]]['excludes']
